...
class MainView(SuperuserRequiredMixin, TemplateView):
    ...
    def get_context_data(self):
        ...
        ctx['all_preorders'] = PreorderPosition.objects.filter(preorder__is_canceled=False).count()
        ctx['redeemed_preorders'] = PreorderPosition.objects.filter(preorder__is_canceled=False, transaction_positions__isnull=False).count()
